package ru.job4j.block4;


import java.util.Arrays;
import java.util.Comparator;
import java.util.List;

/**
 * Применение анонимного класса на примере одноразового Comparator
 */
public class AttachmentSort {
    public static void main(String[] args) {
        List<Attachment> attachments = Arrays.asList(
                new Attachment("image 5", 100),
                new Attachment("image 2", 34),
                new Attachment("image 3", 13)
        );
/**
 * Создаем анонимный класс дял Comparator для сравнения по size
 */
        Comparator<Attachment> comparatorSize = new Comparator<Attachment>() {
            @Override
            public int compare(Attachment o1, Attachment o2) {
                return o1.getSize() - o2.getSize();
            }
        };
        attachments.sort(comparatorSize);
        System.out.println(attachments);
/**
 * Создаем анонимный класс дял Comparator для сравнения по name
 */
        Comparator<Attachment> comparatorName = new Comparator<Attachment>() {
            @Override
            public int compare(Attachment o1, Attachment o2) {
                return o1.getName().compareTo(o2.getName());
            }
        };
        attachments.sort(comparatorName);
        System.out.println(attachments);
    }
}